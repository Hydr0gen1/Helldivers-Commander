...
def test_snapshot_rows_are_built_for_batch_persistence() -> None:
    from datetime import datetime, timezone

    from app.models.domain import Planet, War
    from app.persistence import snapshot_rows

    war = War(war_id=1, time=datetime(2026, 6, 6, tzinfo=timezone.utc), impact_multiplier=1.5)
    planets = [Planet(index=7, name="Test", health=900, max_health=1000, owner=2, players=12, regen_per_second=0.5, liberationPct=10)]

    rows = snapshot_rows(war, planets, [{"planetIndex": 7, "type": 13}])
