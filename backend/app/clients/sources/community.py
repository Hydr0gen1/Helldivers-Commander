...
def _normalize_faction_id(value: Any, *, default: int = 1) -> int:
    if value is None:
        return default
    ...
    if isinstance(value, str):
        ...
        if stripped.isdigit():
            return int(stripped)
        return _FACTION_NAME_TO_ID.get(stripped.casefold(), default)
    return default


def _normalize_faction_name(value: Any, *, fallback_id: int = 1) -> str:
    if isinstance(value, str) and not value.strip().isdigit():
        faction_id = _normalize_faction_id(value, default=fallback_id)
        return faction_name(faction_id)
    return faction_name(_normalize_faction_id(value, default=fallback_id))
...
class CommunitySource:
    ...
    def normalize_planet(self, raw: Any) -> Planet:
        ...
        owner = _normalize_faction_id(_pick(data, "owner", "currentOwner", "initialOwner", default=1), default=1)
        current_owner = _normalize_faction_name(_pick(data, "currentOwner", "current_owner", default=owner), fallback_id=owner)
