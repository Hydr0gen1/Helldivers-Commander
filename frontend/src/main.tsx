  ...
  useEffect(() => {
    const refresh = async (): Promise<void> => {
      try {
        ...
        setWar(warResponse);
        try {
          const gambitResponse = await api.gambits();
          setGambits(gambitResponse.gambits);
        } catch {
          setGambits([]);
        }
        try {
    ...
    <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <GalaxyMap
          ...
          gambits={gambits}
          showGraphIntel={showGraphIntel}
          onToggleGraphIntel={() => setShowGraphIntel((value) => !value)}
        />
        <div className="space-y-6">
          <PlanetPanel planet={selected} gambits={showGraphIntel ? gambits : []} />
